...
//#include "___TWI_IO.h"
#include "LiquidCrystalNew_TWI2.h"

//1/2 chip with software SPI GPIO (3 wire)
LiquidCrystalNew_TWI2::LiquidCrystalNew_TWI2(const byte adrs,const byte chip){
	if (chip == 0 || chip == 255){
		_en2 = 255;
		_multipleChip = 0;
	} else {
...
void LiquidCrystalNew_TWI2::begin(byte cols, byte lines, uint8_t dotsize) {
...
void LiquidCrystalNew_TWI2::initChip(uint8_t dotsize, byte witchEnablePin) {  
...
void LiquidCrystalNew_TWI2::send(uint8_t value, byte mode) {
	byte en = _en1;
...
void LiquidCrystalNew_TWI2::write4bits(byte value) {  //still used during init
...
void LiquidCrystalNew_TWI2::setDataMode(byte mode) {
	bitWrite(_theData,LCDPIN_RS,mode);
}

void LiquidCrystalNew_TWI2::pulseEnable(byte witchEnablePin) {
	writeGpio(_theData | witchEnablePin);   // En HIGH
	writeGpio(_theData & ~witchEnablePin);  // En LOW
}


void LiquidCrystalNew_TWI2::writeGpio(byte value){
...
void LiquidCrystalNew_TWI2::backlight(byte val){
#ifdef BACKGND_LGHTINV
	_backLight = !val;
#else
	_backLight = val;
